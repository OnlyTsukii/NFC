...
func ATCmdRespFrameToBytes(f *ATCmdRespFrame) []byte {
	bs := []byte{
		f.Delimiter,
		f.LenStart,
		f.LenEnd,
	...
	bs = append(bs, f.Command...)
	bs = append(bs, f.Status)
	bs = append(bs, f.Data...)
	bs = append(bs, f.CheckSum)
	return bs
}

func TransmitStatusFrameToBytes(f *TransmitStatusFrame) []byte {
	bs := []byte{
		f.Delimiter,
		f.LenStart,
		f.LenEnd,
	...
	bs = append(bs, f.RetryCount)
	bs = append(bs, f.DeliveryStatus)
	bs = append(bs, f.DiscoveryStatus)
	bs = append(bs, f.CheckSum)
	return bs
}

func ReceivePacketFrameToBytes(f *ReceivePacketFrame) []byte {
	bs := []byte{
		f.Delimiter,
		f.LenStart,
		f.LenEnd,
